...
use crate::{GrevmError, ParallelState, SkipReason, TxExecutionOutcome, TxId};
            ...
            match self.state_ref().basic_ref(tx_env.caller) {
                Ok(info) => {
                    ...
                    match tx_env.nonce.cmp(&expect) {
                        Ordering::Greater => {
                            self.results
                                .push(TxExecutionOutcome::Skipped(SkipReason::NonceTooHigh));
                            return true;
                        }
                        Ordering::Less => {
                            self.results.push(TxExecutionOutcome::Skipped(SkipReason::NonceTooLow));
                            return true;
                        }
                        _ => {}
                    }
...
mod tests {
    ...
    #[test]
    fn missing_sender_uses_zero_nonce() {
        ...
        assert_eq!(
            commit.take_result(),
            vec![TxExecutionOutcome::Skipped(SkipReason::NonceTooHigh)]
        );
    }
